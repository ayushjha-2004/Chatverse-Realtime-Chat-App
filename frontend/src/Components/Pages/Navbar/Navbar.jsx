...
const Navbar = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      socket.emit("disconnected-user", { senderId: userId });
      ...
      >
        <div className="my-8">
          ...
          <Link to={`/${userId}/getChat`} onClick={() => setOpen(false)}>
            <div className="flex justify-center items-center my-4 cursor-pointer rounded-full bg-green-500 transition-all duration-300 ease-in-out  hover:bg-green-600 p-3">
              <FontAwesomeIcon icon={faCommentsRegular} size="xl" />
              <h3 className={`${!sidebarOpen && "hidden"} p-2 text-xl text-white`}>
                Chat
              </h3>
            </div>
          </Link>
        ...
        <div>
          ...
          <div
            className="flex justify-center items-center my-4 cursor-pointer rounded-full bg-green-500 transition-all duration-300 ease-in-out  hover:bg-green-600 p-3"
