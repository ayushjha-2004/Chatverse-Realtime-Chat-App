...
export const joinGroup = async (req, res) => {
  try {
    ...
    await user.save();
    return res.status(200).json({message: "Group joined",group});
  } catch (error) {
    console.log(error);
    return res.status(500).json(error);
  }
...
export const leaveGroup = async (req, res) => {
  try {
    ...
    if (group.members.length === 0) {
      ...
      return res.status(200).json({ message: "Group left and deleted." });
    }

    // Save changes to the group and user
    await group.save();
    ...
    return res.status(200).json({ message: "Group left", group });
  } catch (error) {
    console.log(error);
    return res.status(500).json(error);
  }
